...
namespace GISEditor.EditorTool.Tool
{
    public class SelectFeatureToolClass : ITool, ICommand
    {
        ...
        public IEngineEditLayers EngineEditLayers
        {
            get { return m_EngineEditLayers; }
            set { m_EngineEditLayers = value; }
        }
        ...
        public void OnMouseDown(int button, int shift, int x, int y)
        {
            ...
            { 
                case esriGeometryType.esriGeometryPoint:
                    //将像素距离转化为地图单位距离
                    dLength = MapManager.ConvertPixelToMapUnits(m_activeView, 8);
                    pSpatialFilter.SpatialRel = esriSpatialRelEnum.esriSpatialRelContains;
                    break;
                case esriGeometryType.esriGeometryPolygon:
                    dLength=MapManager.ConvertPixelToMapUnits(m_activeView,4);
            ...
            //根据过滤条件进行查询
            IFeatureCursor pFeatureCursor = pFeatCls.Search(pQueryFilter, true);
            ...
            m_Map.SelectByShape(pGeometry, null, true);
