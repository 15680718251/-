...
WHERE SDO_WITHIN_DISTANCE(B.shape, 
    ...
    id2);
        ...
        public void updateOSCLine(OracleConnection conn, string table, string id, string wkt)
        {
            ...
            {
                string sql = string.Format("update {0} set shape={1} where objectid={2}", table, "sdo_geometry ( :geom,null )", id);
        ...
        public void insertInsectPoint(OracleConnection conn, string table1, string id1, string table2, string id2, string wkt, string type)
        {
            ...
            string sql2 = string.Format("({0},{1},'{2}','{3}',sdo_geometry ('{4}',null),'{5}','{6}','{7}')", id1, id2, feature1, feature2, wkt, type, table1, table2);
            strSql.Append(sql2);
        ...
        {
            string queryString = String.Format(
                "select sdo_geom.relate(c_a.SHAPE,'touch',MDSYS.SDO_GEOMETRY(2001 , null , MDSYS.SDO_POINT_TYPE ({0},NULL),  NULL ,  NULL),0.000001) from {1} c_a where c_a.OBJECTID={2}"
                , point
                , table1
                , id1);
            OracleCommand command = new OracleCommand(queryString, myConnection);
        ...
        {
            string queryString = String.Format(
                "select sdo_geom.relate(MDSYS.SDO_GEOMETRY(2002,null,NULL,MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1),MDSYS.SDO_ORDINATE_ARRAY({0},{1})),'on',c_a.SHAPE,0.000001) from {2} c_a where c_a.OBJECTID={3}"
                , dian1
                , dian2
                , table1
                , id1);
        ...
        public static string getType(OracleConnection myConnection, string dian1, string dian2, string table1, string id1)
        {
            string type = null;
            string queryString = String.Format(
                "select sdo_geom.relate(MDSYS.SDO_GEOMETRY(2002,null,NULL,MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1),MDSYS.SDO_ORDINATE_ARRAY({0},{1})),'determine',c_a.SHAPE,0.000001) from {2} c_a where c_a.OBJECTID={3}"
                , dian1
                , dian2
                , table1
                , id1);
