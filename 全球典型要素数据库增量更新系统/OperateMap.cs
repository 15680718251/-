...
namespace 全球典型要素数据库增量更新系统
{
    class OperateMap
    {
        ...
        public IFeatureLayer GetFeatLyrByName2(IMap pMap, string sFeatLyrName)
        {
            IFeatureLayer pFeatLyr = null;
            try
            {
                ...
                for (int i = 0; i < pMap.LayerCount; i++)
                {
                    ...
                    if (pLayer is IGroupLayer)
                    {
                        ...
                        {
                            pLayer = pGroupLayer.get_Layer(j);
                            pFeatLyr = GetFeatLyrByName(pLayer, sFeatLyrName);
                            if (pFeatLyr != null)
                            {
                                return pFeatLyr;
        ...
        public string getPath(string path)
        {
            ...
            {
                if (path.Substring(t, 15) == "全球典型要素数据库增量更新系统")
                {
                    break;
                }
            }
            string name = path.Substring(0, t + 15);
            return name;
        }
        #endregion
