...
namespace GIS.mm_Conv_Symbol
{
    public class SymbolAlgorithm
    {
        ...
        public static bool line_line_intersectpoint(float x1, float y1, float x2, float y2,
        ...
        public static PointF[] gen_paraline(float udoffset, PointF[] vertex, float[] ver_len, float[] rad)
        {
            ...
            for (int j = 0, index = 1; j < tempvertex.Length - 3; j = j + 2, index++)
            {
                ...
                if (line_line_intersectpoint(tempvertex[j].X, tempvertex[j].Y, tempvertex[j + 1].X, tempvertex[j + 1].Y,
                ...
                {
                    paral_vertices[index].X = tempvertex[j + 1].X;
                    paral_vertices[index].Y = tempvertex[j + 1].Y;
                }
            }
