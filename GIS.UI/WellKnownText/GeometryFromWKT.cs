...
namespace GIS.UI.WellKnownText
{
    ...
    public class GeometryFromWKT
    {
        ...
        private static List<GeoPoint> GetCoordinates(WktStreamTokenizer tokenizer)
        {
            ...
            GeoPoint externalCoordinate = new GeoPoint();
            GeoPoint internalCoordinate = new GeoPoint();
            ...
            while (nextToken == ",")
            {
                internalCoordinate = new GeoPoint();
        ...
        {
            GeoMultiPoint mp = new GeoMultiPoint();
            string nextToken = GetNextEmptyOrOpener(tokenizer);
            if (nextToken == "EMPTY")
                return mp;
            mp.Points.Add(new GeoPoint(GetNextNumber(tokenizer), GetNextNumber(tokenizer)));
            nextToken = GetNextCloserOrComma(tokenizer);
            while (nextToken == ",")
            {
                mp.Points.Add(new GeoPoint(GetNextNumber(tokenizer), GetNextNumber(tokenizer)));
        ...
        {
            return new GeoLineString(GetCoordinates(tokenizer));
        }
