...
namespace GIS.UI.Forms
{
    ...
    public partial class LineElementMatchUpdate : Form
    {
        ...
        public LineElementMatchUpdate()
        {
            InitializeComponent();
        }
        public LineElementMatchUpdate(string conStr, AxMapControl axMapcontrol, AxTOCControl axTOCControl)
        {
            InitializeComponent();
            this.conStr = conStr;
        ...
        /// <param name="e"></param>
        private void LineFeatureUpdategBox_Enter(object sender, EventArgs e)
        {
        }
        /// <summary>
        /// 线要素匹配更新容器触发事件 
        ...
        private void TrafficUpdategBox_Enter(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("对不起，您未连接数据库或者您选择的数据库中无线数据表！");
            }
        }
        /// <summary>
        /// 获取所有的表名
        ...
        private List<string> getTableNames(string conString)
        {
            ...
            try
            {
                ...
                string sqlString = "select distinct table_name  from user_col_comments where lower(table_name) like '%line%' or  table_name like '%currentlinedata%'or table_name like '%incrementlinedata%'or table_name like '%traffic%'or table_name like '%water%'or table_name like '%vegetation%'or table_name like '%soil%'or table_name like '%residential%'order by  lower(table_name) ASC";
        ...
        public DataTable getDataByBuffer(string tableName, string geometry_wkt)
        {
            ...
            try
            {
                ...
                {
                    sql = tableName.Equals("TRAFFIC_LINE") ? string.Format("select objectid,osmid,versionid,starttime,endtime,nationelename,nationcode,sdo_geometry.get_wkt(shape)geometry_wkt from {0} where SDO_WITHIN_DISTANCE(shape,sdo_geometry ('{1}',4326) ,'distance={2},unit=m')='TRUE'", tableName, geometry_wkt, bufferRadius)
                        : string.Format("select objectid,osmid,versionid,starttime,endtime,nationelename,nationcode,sdo_geometry.get_wkt(shape)geometry_wkt from {0} where SDO_WITHIN_DISTANCE(geom,sdo_geometry ('{1}',4326)) ,'distance={2},unit=m')='TRUE'", tableName, geometry_wkt, bufferRadius);//得到缓冲区内对象
                    ...
                    bufferData.Columns.Add("starttime");
                    bufferData.Columns.Add("endtime");
                    ...
                    using (OracleDataReader rd = helper.queryReader(sql))
                    {
                        while (rd.Read())
                        {
                            ...
                            if (tableName.Equals("TRAFFIC_LINE"))
                            {
                                ...
                                bufferDataRow["starttime"] = rd["starttime"].ToString();
                                bufferDataRow["endtime"] = rd["endtime"].ToString();
        ...
        public DataTable getDataAll(string tableName)
        {
            ...
            {
                OracleDBHelper helper = new OracleDBHelper();
                if ("TRAFFIC_NEWLINE".Equals(tableName))
                {
                    sql = string.Format("select objectid,osmid,versionid,starttime,endtime,nationelename,nationcode,sdo_geometry.get_wkt(shape)geometry_wkt from {0} order by osmid,versionid,objectid ", tableName);
                ...
                using (OracleDataReader rd = helper.queryReader(sql))
                {
                    while (rd.Read())
                    {
                        ...
                        if ("TRAFFIC_NEWLINE".Equals(tableName))
                        {
                            ...
                            tableRow["starttime"] = rd["starttime"].ToString();
                            tableRow["endtime"] = rd["endtime"].ToString();
        ...
        /// 线要素匹配
        /// </summary>
        /// <param name="tableNameA">增量数据表</param>
        /// <param name="tableNameB">基态数据表</param>
        ...
        public void LineMatching(string tableNameA, string tableNameB, string conStringA, string conStringB)
        {
            try
            {
                ...
                for (int i = 0; i < tableCount; i++)//遍历循环表中数据
                {
                    ...
                    string osmid_V = tableA.Rows[i]["osmid"].ToString() + "_" + tableA.Rows[i]["versionid"].ToString();//确定一个线对象
                    string geomStr_wktA = tableA.Rows[i]["geometry_wkt"].ToString();
                    string timesA = tableA.Rows[i]["starttime"].ToString();
                    string endtimeA = tableA.Rows[i]["endtime"].ToString();
                    ...
                    if (tableB.Rows.Count > 0)
                    {
                        if (!overallMatching(tableB, osmid_V, geomStr_wktA, nationEleNameA, conStringB, tableNameB))//整体匹配没有成功，则进行局部匹配
                        {
                            ...
                            { //如果局部匹配也没成功，则说明基态数据中不存在与该对象可以匹配的对象，将其保留                               
                                ...
                                dataRetain(tableA.Rows[i]["osmid"].ToString(), int.Parse(tableA.Rows[i]["versionid"].ToString()), tableNameA, tableNameB, conStringA, conStringB);
                            }
                        }
                    }
                    else
        ...
        /// <param name="conStringA"></param>
        /// <param name="conStringB"></param>
        public void dataRetain(string osmid, int versionid, string tableNameA, string tableNameB, string conStringA, string conStringB)
        {
            try
            {
                ...
                string nationEleNameB = "", nationCodeB = "",tagsB="";
                //string sql = string.Format("select osmid,username,userid,versionid,starttime,osmkey,osmvalue,nationELeName,nationCode,elementTypeID,changeset,sdo_geometry.get_wkt(shape)geometry_wkt from {0} where osmid='{1}' and versionid ={2} ", tableNameA, osm_id, version_id);
                string sql = string.Format("select osmid,username,userid,versionid,starttime,fc,dsg,tags,nationeLename,nationcode,changeset,sdo_geometry.get_wkt(shape)geometry_wkt from {0} where osmid='{1}' and versionid ={2} ", tableNameA, osmid, versionid);
                ...
                endtimeB.ToString("YYYY-MM-DD HH24:MI:SS");
                ...
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    using (OracleDataReader rd = cmd.ExecuteReader())
                    {
                        while (rd.Read())
                        {
                            ...
                            useridB = int.Parse(rd["userid"].ToString());
                            versionidB = int.Parse(rd["versionid"].ToString());
                            usernameB = rd["username"].ToString();
                            timesB = rd["starttime"].ToString();//to_timestamp('2010-10-01 21:30:50', 'YYYY-MM-DD HH24:MI:SS')sql
                            osmkeyB = rd["fc"].ToString();
                            osmvalueB = rd["dsg"].ToString();
                            tagsB=rd["tags"].ToString ();
                ...
                {
                    sql = string.Format("insert into {0} (osmid,versionid,starttime,endtime,changeset,userid,username,fc,dsg,tags,trustvalue,userreputation,shape,source,nationelename,nationcode)(select {1}, {2},to_timestamp('{3}', 'YYYY-MM-DD\"T\" HH24:MI:SS\"Z\"'),{4},'{5}',{6},'{7}','{8}','{9}','{10}',{11}.trustvalue,{11}.userreputation,{11}.shape,{11}.source, '{12}', '{13}'  FROM {11} where exists (select osmid FROM {0} where osmid={1})) ", tableNameB, osmidB, versionidB, timesB, endtimeB, changesetB, useridB, usernameB, osmkeyB, osmvalueB, tagsB, tableNameA, nationEleNameB, nationCodeB);
                    using (OracleCommand cmd = new OracleCommand(sql, conn))
                    {
                        cmd.ExecuteNonQuery();
                        Console.WriteLine("成功插入数据：osmid=" + osmidB);
                    }
                }
                else
                {
                    ...
                    //sql = string.Format("insert into {0} (objectid,osmid,userid,username,versionid,starttime,endtime,changeset,osm_key,osm_value,trustvalue,userreputation,shape,source,nationEleName,nationCode,elementTypeID)values({1},{2},{3},'{4}',{5},to_timestamp('{6}', 'YYYY-MM-DD\"T\" HH24:MI:SS\"Z\"'),'{7}','{8}',sdo_geometry ('{9}',4326),'{10}','{11}',{12},'{13}')", tableNameB, objectidB, osmidB, useridB, usernameB, versionidB, timesB, osmkeyB, osmvalueB, geomB, nationEleNameB, nationCodeB, elementTypeIDB, changesetB);
                    //using (OracleCommand cmd = new OracleCommand(sql, conn))
                    //{
                    //    cmd.ExecuteNonQuery();
                    //    Console.WriteLine("成功插入数据：objectid=" + objectidB);
        ...
        public Boolean overallMatching(DataTable bufferDataTable, string osmid_v, string geomStr_wktA, string nationEleNameA, string conStringB, string tableNameB)
        {
            ...
            try
            {
                ...
                for (int j = 0; j < bufferDataTableCount; j++)//循环遍历基态线对象缓冲区内对象
                {
                    ...
                    string nationcodeB = bufferDataTable.Rows[j]["nationcode"].ToString();
        ...
        private void roadMatchingBT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string tablename=tableNameB;
                showmap.showMap(tablename, axMapcontrol);
